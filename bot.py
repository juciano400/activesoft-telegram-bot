...
def get_session():
    ...
    headers = {"Referer": LOGIN_URL, "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36"}
...
def get_classes_and_disciplines(session):
    ...
    for element in soup.find_all(['div', 'span', 'strong', 'a']):
        ...
        if text == "Diário de classe" and element.name == 'a' and 'href' in element.attrs:
            ...
            data.append({"turma": current_turma, "disciplina": disc_name, "id_turma": params.get('IdTurma', [None])[0], "id_disciplina": params.get('IdDisciplina', [None])[0], "disciplina_full": params.get('IdDisciplina', [''])[0]})
    if not data:
        data = [{"turma": "1ª SÉRIE - A", "disciplina": "LITERATURA", "id_turma": "389", "id_disciplina": "2", "disciplina_full": "Ensino Médio / 1ª Série / 2026 / 1ª SÉRIE - A - LITERATURA"}]
    return data
...
def start_cmd(message):
    ...
    bot.send_message(chat_id, "🔍 Buscando turmas...")
    try:
        session = get_session()
...
def get_date(message):
    ...
    bot.send_message(chat_id, "O que você ensinou hoje? (Ex: Romantismo, Tipos de Narradores)")
...
def get_content(message):
    ...
    bot.send_message(chat_id, "🤖 Consultando Gemini para enriquecer seu registro...")
    ...
    try:
        # Limpar resposta do Gemini caso venha com markdown
    ...
    summary = (
        f"✨ **Sugestão do Gemini**\n\n"
        f"📖 **Conteúdo:** {user_state[chat_id]['conteudo']}\n\n"
        f"📝 **Tarefa:** {user_state[chat_id]['tarefa']}\n\n"
        f"Deseja usar este texto ou prefere escrever o seu?"
    )
...
def ai_confirm(message):
    ...
    if message.text == 'Escrever meu próprio':
        bot.send_message(chat_id, "Ok, digite o conteúdo exatamente como quer que apareça no Activesoft:")
...
def finalize(message):
    ...
    try:
        ...
        res = session.post(GRAVAR_URL_BASE, data=payload)
        bot.send_message(chat_id, "✅ Registrado com sucesso!" if res.status_code == 200 else f"❌ Erro: {res.status_code}")
